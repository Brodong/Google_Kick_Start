...
# Last Modified: 2020-11-12 15:08:39
#
 ***********************************************/
#include <iostream>
#include <stdio.h>
        ...
        for(i=0; i<C; i++)
        {
            for(j=R-1; j>0; j--)
            {
                ...
                else
                {
                    ...
                    for(k=0; k<G[alp[i][j]-'A'].size; k++)
                    {
                        if(G[alp[i][j]-'A'][k] == alp[i][j-1]-'A')
                            break;
                    }
                    if(k == G[alp[i][j]-'A'].size)
                    {
                        G[alp[i][j]-'A'].push(alp[i][j-1]-'A');
                        // 入度加1            
                        indegree[alp[i][j-1]-'A']++;
                    }
                }
            }
        }
